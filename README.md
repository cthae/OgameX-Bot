...
* [ ] Test main features [in progress]
  ...
  * [x] Expos are being sent properly though there are slight quirks. It sends them manually one after another to avoid them all coming back together to prevent a timed destruction of all fleets. But it doesn't collect debriss, which is the main point of expos later on. But it lacks the option to make use of multi-expo feature. They can ban you on suspicion of scripting if you send too many expos manually, so I suggest not using this until multi-expo feature is there.
